...
class NavigationController {
  ...
  constructor(owner, domRoot, domRootD3, nSteps) {
    ...
    this.domRoot.find("#jmpStepBack").click(() => {
      this.owner.stepBack();
      this.interactionLogs.push(`Interaction with Python Tutor (button= Back, date=${new Date().toLocaleString()})`);
      // window.beforeViz.stepBack();
      // window.afterViz.stepBack();
    });
    this.domRoot.find("#jmpStepFwd").click(() => {
      this.owner.stepForward();
      this.interactionLogs.push(`Interaction with Python Tutor (button= Forward, date=${new Date().toLocaleString()})`);
